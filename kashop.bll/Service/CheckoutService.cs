...
namespace kashop.bll.Service
{
    public class CheckoutService : ICheckoutService
    {
        ...
        public async Task<CheckoutResponse> ProcessPaymentAsync(CheckoutRequest request, string userId)
        {
            ...
            if (request.PaymentMethod == PaymentMethodEnum.Cash) {
                ...
                var options = new SessionCreateOptions
                {
                    ...
                    Mode = "payment",
                    SuccessUrl = $"http://localhost:5124/api/checkouts/success?session_id={{CHECKOUT_SESSION_ID}}",
                    CancelUrl = $"http://localhost:5124/api/checkouts/cancel",
        ...
        public async Task<CheckoutResponse> HandelSuccessAsync(string sessionId)
        {
            var service = new SessionService();
